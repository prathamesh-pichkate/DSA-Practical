...
void printMirrorTraversal(Node* root)
{
    if (root == NULL)
    {
        return;
    ...
    {
        cout << root->data << " ";
        printMirrorTraversal(root->right); // Traverse right first
        printMirrorTraversal(root->left); // Then traverse left
    }
}
...
int main()
{
    ...
    mirror(root);
    cout << "Displaying the elements of the mirrored BST-\n";
    printMirrorTraversal(root);
    cout << endl;
